...
The instances within each bag are reordered based on their IIS ranking, denoted as $$ V_i' = \{ v_{(i,j)}' | IIS(v_{(i,1)}') \geq IIS(v_{(i,2)}') \geq \ldots \geq IIS(v_{(i,N_i)}') \} $$. Then, these instances are evenly interleaved into $$ M $$ pseudo bags using the modulus function, ensuring that $$ v_{(i,j)}' $$ satisfies $$ j \equiv k \, (\text{mod} \, M) $$, resulting in each pseudo bag $$ X_{(i,k)}^{pse} $$ being represented as samples from $$ V_i' $$. By fixing the parameters $$ \theta $$ of the MIL model, the optimization of $$ \epsilon $$ can be approximated as:
